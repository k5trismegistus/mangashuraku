  ...
  return () => {
    return new Promise((resolve, reject) => {
      extract(ctx)
        ...
        .then(registerToDb)
        .then(cleanUp)
        .then((ctx: ImportContext) =>
          console.log(`finished: ${basename(zipFilePath)}`)
        )
        .then(() => resolve())
        .catch(err => {
          console.error(err)
...
readdir('import/', (err, files) => {
  ...
  const zipList = files.filter(
    file => file.endsWith('.zip') && statSync(join('import', file)).isFile()
  )

  zipList
    .map(zipFile => importArchive(join(importDir, zipFile)))
    .reduce((m, p) => m.then(p), Promise.resolve({}))
